...
static int parse_config(config_entry_list *list, const char *path);
...
config_entry_list
  : config_entry_list config_entry
    {
      list->next = new_config_entry_list();
      list = list->next;
    }
  | config_entry
    {
...
{
  config_entry_list *list = new_config_entry_list();
  int retval = parse_config(list, argv[1]);
